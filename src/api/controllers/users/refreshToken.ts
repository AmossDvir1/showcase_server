...
export const refreshToken = async (req: Request, res: Response) => {
  ...
  if (!sessionId) {
    return res.status(401).json({ message: "No sessionId provided.", error: "noSessionId" });
  }
  ...
  try {
    ...
    const session = await Session.findOne({ userId, token: refreshToken, _id: sessionId  });
    if (!session) {
    ...
    return res.status(200).json({ success: true, accessToken: newAccessToken, sessionId:session._id });
  } catch (err: any) {
    console.log("Error in refreshToken endpoint: ", err);
    res.status(401).json({ message: "Failed to refresh tokens.", error: err.message });
  }
};
