...
const uploadPicture = async (req: Request, res: Response, purpose: ImagePurpose) => {
  ...
  try {
    ...
    const base64Data = data.imageStringBase64.replace(/^data:image\/\w+;base64,/, "");
    ...
    return res.status(400).json({ error: "Invalid image format", message: "Unsupported image format" });
  }
};
